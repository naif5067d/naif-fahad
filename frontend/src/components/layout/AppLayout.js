...
import { LayoutDashboard, FileText, CalendarDays, Clock, FileSignature, Users, Settings, Shield, Menu, X, Sun, Moon, Globe, ChevronDown, Check, MapPin, Package, Wallet, Wrench, FileCheck, Receipt, AlertTriangle, UsersRound, LogOut, History, ClipboardList, Hammer, Activity, Maximize, Minimize, ClipboardCheck, Book, Volume2, Rocket, Fingerprint, ShieldCheck } from 'lucide-react';
...
const NAV_ITEMS = {
  ...
  stas: [
    ...
    // 📊 التقارير
    'executive', 'maintenanceTracking', 'ats',
    // 🔐 مراقبة الأجهزة والأمان
    'deviceMonitoring', 'security',
          ...
          <div className="flex items-center justify-between px-3 md:px-6 h-12 md:h-16" style={{ overflow: 'visible' }}>
            ...
            <div className="flex items-center gap-1 md:gap-3 flex-shrink-0" style={{ overflow: 'visible' }}>
              ...
              <button 
                ...
                onClick={toggleLang} 
                className="w-[44px] h-[44px] md:w-10 md:h-10 flex items-center justify-center rounded-xl hover:bg-muted active:bg-muted/80 text-muted-foreground transition-colors flex-shrink-0"
                title={t('lang.toggle')}
              >
                <Globe size={18} />
              </button>
              ...
              <div className="relative" ref={logoutRef}>
                <button 
                  ...
                  className="w-[44px] h-[44px] md:w-10 md:h-10 flex items-center justify-center rounded-xl hover:bg-destructive/10 hover:text-destructive active:bg-destructive/15 text-muted-foreground transition-colors flex-shrink-0"
