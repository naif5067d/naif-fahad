...
async def activate_ramadan_mode(req: RamadanModeRequest, user=Depends(require_roles('stas'))):
    ...
    # حفظ الأوقات الأصلية وتحديث الأوقات الجديدة
    ...
    for loc in locations:
        update_data = {
            ...
            "work_start": req.work_start,
            "work_end": req.work_end,
            "daily_hours": req.daily_hours,  # 6 ساعات في رمضان
            "ramadan_updated_at": now
        }
        
        # حفظ الأوقات الأصلية إذا لم تكن محفوظة
        if not loc.get('original_work_start_saved'):
            update_data["original_work_start_saved"] = loc.get('work_start', '08:00')
            update_data["original_work_end_saved"] = loc.get('work_end', '17:00')
            update_data["original_daily_hours_saved"] = loc.get('daily_hours', 8.0)
    ...
    return {
        ...
        "note_ar": f"تم تحديث أوقات جميع مواقع العمل إلى {req.work_start} - {req.work_end} ({req.daily_hours} ساعات)"
    }
