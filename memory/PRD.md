...
### Phase 39.2: نظام صلاحيات تعديل الحضور ✅ (2026-02-20)

**النظام الجديد - صلاحيات التعديل:**

| الدور | الصلاحيات |
|-------|-----------|
| المشرف (supervisor) | يطلب تعديل → ينتظر موافقة سلطان |
| سلطان | موافقة/رفض/تعديل → قرار نهائي ونافذ |
| STAS | أرشيف سنوي للمراجعة والتدقيق |

**سير العمل:**
```
1. المشرف يطلب تعديل حالة موظف تحت إشرافه
   ├── يظهر له تحذير: "عزيزي [الاسم]، تعديلك للحالة يعني تحملك لمسؤوليتها"
   └── يجب الإقرار (supervisor_acknowledgment=true)

2. الطلب يظهر لسلطان في قائمة "طلبات التعديل المعلقة"
   └── يعرض: الموظف، الحالة الأصلية، المطلوبة، السبب، المشرف

3. سلطان يقرر:
   ├── approve → موافقة على الحالة المطلوبة
   ├── reject → رفض التعديل (يبقى على الحالة الأصلية)
   └── modify → موافقة مع تعديل الحالة

4. القرار يُطبق مباشرة على daily_status
5. يُحفظ في أرشيف STAS السنوي (stas_annual_archive)
6. إشعارات ترسل للمشرف والموظف
```

**Collections الجديدة:**
- `attendance_corrections`: طلبات التعديل من المشرفين
- `stas_annual_archive`: أرشيف القرارات السنوي

**APIs الجديدة:**
```
POST /api/team-attendance/{emp}/request-correction/{date}
     → المشرف يطلب تعديل

GET  /api/team-attendance/pending-corrections
     → سلطان يرى الطلبات المعلقة

POST /api/team-attendance/correction/{id}/decide
     → سلطان يقرر (approve/reject/modify)

GET  /api/team-attendance/stas-archive?year=2026
     → STAS يراجع الأرشيف السنوي

GET  /api/team-attendance/corrections-history/{emp}
     → سجل تعديلات موظف معين
```

**الربط بالنظام:**
- ✅ يُحفظ في `daily_status.corrections[]` مع تفاصيل الطلب والقرار
- ✅ يُحفظ في `stas_annual_archive` للتدقيق
- ✅ الإشعارات ترسل لجميع الأطراف
- ✅ مرتبط بنظام الخصومات والمخالصة (يؤثر على حساب الغياب)

**الملفات المُحدّثة:**
- `/app/backend/routes/team_attendance.py` - جميع الـ APIs الجديدة
